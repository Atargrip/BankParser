    ...
    def can_parse(self, file_bytes: bytes) -> bool:
        try:
            with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
                ...
                return "FORTEBANK" in first_page_text and "ВЫПИСКА ПО ЛИЦЕВОМУ СЧЕТУ" in first_page_text
        except Exception:
            return False
    ...
    def parse(self, file_bytes: bytes) -> ParseResult:
        ...
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            if len(pdf.pages) > 0:
                ...
                currency_match = re.search(r"Валюта[^:\n]*:\s*([A-Z]{3})", first_page_text, flags=re.IGNORECASE)
                if currency_match:
            ...
            for page in pdf.pages:
                ...
                for table in tables:
                    for row_idx, row in enumerate(table):
                        ...
                        try:
                            date_cell = clean_row[1]
                            date_value = date_cell.split()[0] if date_cell else ""
